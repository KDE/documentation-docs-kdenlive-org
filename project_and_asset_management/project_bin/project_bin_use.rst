...
.. list-table::
   ...
   * - **3**
     - |tag|\ :guilabel:`Tags Panel` toggles the :ref:`tags<project_bin_use_tags>` panel
   * - **4**
   ...
   * - **5**
     - |view-filter|\ :guilabel:`Filter` switches a :ref:`filter<project_bin_use_filters>` on or off
   * - 
     - |go-down|\ :guilabel:`Filter` opens a pop-up window to :doc:`set filter<project_bin_use_filters>` parameters
   * - **6**
     - Search field to :ref:`search for assets<project_bin_use_searching>` in the bin
   * - **7**
   ...
   * - **9**
     - Folders created by the user (for illustration purposes only). You can turn folders into :ref:`individual bins<project_bin_use_multiple_bins>` that can be added to your workspace and docked.
   * - **10**
