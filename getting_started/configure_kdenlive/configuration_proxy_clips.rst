...
.. .. versionadded:: 26.08 Allowed FFmpeg parameters for proxy clips

Proxy Clips
-----------

.. figure:: /images/getting_started/configure_proxy_clips_2608.webp
   :width: 700px
   :figwidth: 700px
...
:5: :guilabel:`Profile for clips with alpha channel`. Select the profile you want Kdenlive to use for creating the proxy clips with an alpha channel. The available options depend on your HW. Choose one that offers a good compromise of speed and compression. Click on |configure| to manage the available profiles.

:6: :guilabel:`Generate for images larger than`. If checked, proxy clips will be automatically generated for image files larger than the selected amount of pixels wide upon adding them to the project bin.

:7: :guilabel:`Proxy image resize (width)`. Specify the target width of the proxy clips in pixels.

:8: :guilabel:`External proxy clips`. If enabled, Kdenlive will use proxy files of the specified type that are generated by certain cameras already. Examples are GoPro and Sony cameras, and DJI drones. Click on |configure| to manage the available profiles.

:9: :guilabel:`Allowed FFmpeg parameters for proxy clips`. It shows a list of all parameters which FFmpeg allows. If a profile with an unexpected parameter is created or imported, Kdenlive shows a warning and asks the user if they really want to allow these parameters.
