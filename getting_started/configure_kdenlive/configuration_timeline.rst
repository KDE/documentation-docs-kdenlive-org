...
.. figure:: /images/getting_started/configure_timeline_2608.webp
   :width: 700px
   :figwidth: 700px
...
:9: :guilabel:`Fixed centered playhead`. If checked, the playhead remains locked at the center of the timeline while the timeline content scrolls beneath it during playback, scrubbing, and seeking.

:10: :guilabel:`Scroll vertically with mouse wheel (MW), horizontally with Shift+MW`. Changes the default behavior of the mouse wheel (MW) in the timeline.

:11: :guilabel:`Display clip markers comments`. If checked, the comments of clip markers will be displayed in the timeline. If unchecked, the comments will not be displayed.

:12: :guilabel:`Default track height`. Determines the default height of the tracks in the timeline in pixels. The height can be adjusted anytime by dragging the line between tracks. See the chapter about :ref:`resizing_tracks` for more details.

:13: :guilabel:`On import, enable` determines what Kdenlive shall do when an audio clip has multiple audio streams.

:14: :guilabel:`Check if project contains enough tracks`. By default, Kdenlive checks whether the project an audio clip with multiple streams is added to has enough audio tracks to divide the clip into, and if needed asks if it should generate the additionally required audio tracks automatically.

:15: :guilabel:`Developer`. This function is only for developers. It just shows item ids from the timeline model in the timeline.
